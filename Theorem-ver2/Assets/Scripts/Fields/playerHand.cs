...
public class playerHand : NetworkBehaviour
{
    ...
    public void AddCard(GameObject card, Vector3 currPosition)
    {
        ...
        if ((countCards - 1) * size.x > xEnd - xStart)
        {
            ...
            card.GetComponent<Card>().moveCardTo(pos.GetCoord(), pos.GetRotation());
        }
        else
        {
    ...
    private void repositionCards(float distanceCard, float displacement)
    {
        ...
        foreach (Position temp in positions)
        {
            ...
            count++;
            temp.GetCard().GetComponent<Card>().moveCardTo(temp.GetCoord(), temp.GetRotation());
        }
    }

    public Position getPositionOfCard(GameObject card)
    {
        foreach (Position pos in positions)
        {
            if (pos.GetCard() == card)
